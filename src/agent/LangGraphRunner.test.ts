...
import { LangGraphRunner, type LangGraphTransition } from "./LangGraphRunner.js";
...
beforeEach(async () => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miguelito-langgraph-"));
...
function createRunner(provider: LLMProvider, evaluator: EvaluatorProvider, transitions: LangGraphTransition[]) {
  const promptBuilder = {
    build: async () => "system",
...
describe("LangGraphRunner routing", () => {
  it("routes no-tool replies to post_turn for normal user chat", async () => {
    ...
    expect(transitions.every((t) => t.traceId && typeof t.at === "string" && t.runtime === "langgraph")).toBe(true);
  }, 15_000);
