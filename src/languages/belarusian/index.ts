...
export const BelarusianLanguage: LanguageConfig = {
  ...
  promptText: {
    ...
    currentLearnerProfile: ({ words, receptiveWords, productiveWords, errorInfo, weakAreas }) => {
      ...
      if (rec.length > 0) lines.push(`**Рэцэптыўная лексіка**: Уплятай гэтыя выразы натуральна ў свой тэкст, каб праверыць пасіўнае разуменне: ${rec.join(", ")}`);
      if (prod.length > 0) lines.push(`**Прадуктыўная лексіка**: Ствары камунікатыўную патрэбу, каб навучэнец мог сам ужыць АДЗІН з гэтых выразаў. Не чакай выпадковага спантаннага ўжывання і не кажы "выкарыстай гэтае слова", акрамя як апошняй падказкай. Лепей асабістае пытанне, roleplay, перафармуляванне або cloze з паступовымі падказкамі: ${prod.join(", ")}`);
